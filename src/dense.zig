...
pub fn Dense(comptime num_in: usize, comptime num_out: usize, comptime activation: Activation) type {
    return struct {
        ...
        pub fn initWithWeights(self: *@This(), alloc: Allocator, w: []f32, b: []f32) !void {
            ...
            self.weights = try Tensor(f32).empty(.{ num_out, num_in }, alloc);
            self.biases = try Tensor(f32).empty(.{ num_out, 1 }, alloc);
            self.last_outputs = try Tensor(f32).empty(try root.ops.opShape(.MatMul, self.weights.shape, .{ num_in, 1 }), alloc);
            self.activation_outputs = try Tensor(f32).empty(self.last_outputs.shape, alloc);
